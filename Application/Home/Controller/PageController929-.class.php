...
//    public function getTableData1($data,$params_table,$params_table12){
//        $check_obj = false;
...
////                        unset($params_table12);
////                        unset($params_table13);
//                    }
//                    if($check_obj){
...
//    }
    ...
    public function getTableData929($data1,$params_table,$params_table12,$a=0){
        $check_obj = false;
        $params_table13 = $params_table12;
...
//                        }
                        ...
                        $params_table13 .= '|'.$k.' |'.$v['value']['value'].'|'.'1'.'|'.$v['value']['desc'].'|'.$v['value']['type']."\n";
                        $params_table .= '|'.$head.'/'.' |'.$v['value']['type'].'|'.$required.'|'.$v['value']['desc']."\n";
                    }
                }else{
                    $params[$k] = $v['value'];
...
/*    public function getTableData2($data1,$params_table,$params_table12,$a=0){
        $params_table13 = $params_table12;
        $type_arr = ['arr','array','obj','object'];

        $data = $this->checkArray($data1['value']);
        $table_name_type = [];
        foreach ($data as $k => $v) {
            $keys1 = array_keys($v);
            if($keys1[0] == '0'){
            ...
            if(!$a){
                if(!in_array($data1['type'],$type_arr)){
                    ...
                    $params_table .= '|' . '---' . ' |' . $data1['type'] . '|' . '是' . '|' . '结果以'.$data1['type'].'返回' . "\n";
                }
            }
            $a++;
...
//        var_dump($params_array);die;
    ...
    function generateTree929($items,$keys) {
        $tree = array(); //格式化好的树
        $count = [];
        $size = [];
    ...
    function generateTree($items,$keys) {
        $tree = array(); //格式化好的树
        $count = [];
        $size = [];
//        var_dump($items);die;
...
//                        var_dump($params_table);die;
                ...
                $v2['value'] = '['.$v2['value'].']';
//                var_dump($v2);die;
                $params_table = $this->checkPara($v2, $params_table, $head, $params, '', $i++, $keys);
...
/*                        if(!isset($v2['value'])){
                       ...
                       $v2 = $data['value'];
                        $v2['key'] = $head;
                        $v2['up_key'] = $head;
                        $params_table = $this->checkSub($v2,$params_table,$head,$params,$up_keys,$each_arr);
//                    }
...
//                    $params_table .= '|'.$head.'/'.' |'.$data['type'].'|'.$required.'|'.$data['desc']."\n";
//                    break;
                }
//            }
        }
